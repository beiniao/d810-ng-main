...
class OllvmDispatcherInfo(GenericDispatcherInfo):
    ...
    def guess_outmost_dispatcher_blk(
        ...
        while mb.nextb:
            # NOTE: all skip conditions must fall through to `mb = mb.nextb`.
            # A bare `continue` here would loop forever on the same block
            # (this exact bug hung IDA on functions with a high-npred block
            # ending in a flattening jump whose right operand is not mop_n).
            if (
                npred_max < mb.npred()
                and mb.tail
                and mb.tail.opcode in FLATTENING_JUMP_OPCODES
                and mb.tail.r.t == ida_hexrays.mop_n
                and (
                    mb.tail.l.t == ida_hexrays.mop_r
                    or (
...
class Unflattener(GenericDispatcherUnflatteningRule):
    ...
    CONFIG_SCHEMA = GenericDispatcherUnflatteningRule.CONFIG_SCHEMA + (
        ConfigParam("min_entropy", float, 0.3, "Minimum entropy threshold for dispatcher detection"),
        ConfigParam("max_entropy", float, 0.7, "Maximum entropy threshold for dispatcher detection"),
    )
